...
public class RadarTargetIdentifier {
    ...
    public static RadarTargetIdentifier getBestIdentifier(NBTTagCompound nbt, int blockId){
        String identifier;
        String serializedBlockState = null;
        String teID  = nbt.getString("id");
        if(nbt.hasKey("MetaId", 8)){
            identifier = nbt.getString("metaId");
        } else {
            identifier = teID;
            Block block = Block.getBlockById(blockId);
            ResourceLocation blockRes = block.getRegistryName();
            String blockName = (blockRes != null) ? blockRes.toString() : "minecraft:air";
            if (!teID.equals(blockName)) {
                serializedBlockState = blockName;
            }
        }
